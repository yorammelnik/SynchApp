...
public class BigIdService {
	...
	private void convertPrimitve2Complex(ArrayList<ColumnToSynch> columnsToRetrieve) throws SecurityException, IOException {
		LoggerSingelton.getInstance().getLogger().info("Beginning of BigIdService.convertPrimitve2Complex(). columnsToRetrieve: " + columnsToRetrieve);
		...
		LoggerSingelton.getInstance().getLogger().info("End of BigIdService.convertPrimitve2Complex(). columnsToRetrieve: " + columnsToRetrieve);	
	}
	...
	private void convertComplexField2Primitive(ArrayList<CategoryColumnContainer> complianceGroupToUpdate) throws SecurityException, IOException {
		...
		LoggerSingelton.getInstance().getLogger().info("End of BigIdService.convertComplexField2Primitive()");
		...
		for (int i = 0; i < complianceGroupToUpdate.size(); i++) {
			...
			if (! complianceGroupValues.isEmpty()) {
				...
				for (int j = 0; j < complianceGroupValues.size(); j++) {
					...
					LoggerSingelton.getInstance().getLogger().info("postNewCategories. currCategory = " + newCategory);						
					...
					LoggerSingelton.getInstance().getLogger().info("JsonObject when posting a new complianceGroup " + jsonPostObject.toString());
